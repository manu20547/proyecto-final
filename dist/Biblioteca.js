...
const JsonStorageAdapter_1 = require("./JsonStorageAdapter");
...
class Biblioteca {
    ...
    constructor(storage) {
        ...
        this.nextIds = { libro: 1, usuario: 1, prestamo: 1 };
        // Si no se proporciona, usa el adapter de JSON
        this.storage = storage || new JsonStorageAdapter_1.JsonStorageAdapter();
    ...
    load() {
        ...
        if (Array.isArray(data.libros)) {
            for (const lb of data.libros) {
                const libro = Libro_1.Libro.fromJSON(lb);
        ...
        if (Array.isArray(data.usuarios)) {
            for (const us of data.usuarios) {
                const usuario = Usuario_1.Usuario.fromJSON(us);
        ...
        if (Array.isArray(data.prestamos)) {
            for (const pr of data.prestamos) {
                const prestamo = Prestamo_1.Prestamo.fromJSON(pr);
     ...
     * 5. Retorna el Libro
     */
    crearLibro(titulo, autor, año) {
        const id = this.nextIds.libro++;
        const libro = new Libro_1.Libro(id, titulo, autor, año);
    ...
    crearUsuario(nombre) {
        ...
        const usuario = new Usuario_1.Usuario(id, nombre);
    ...
    prestarLibro(libroId, usuarioId) {
        ...
        const prestamo = new Prestamo_1.Prestamo(id, libroId, usuarioId, fecha);
     ...
     */
    listarPrestamos(activosOnly = false) {
