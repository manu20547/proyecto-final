...
function demo() {
    ...
    if (biblioteca.listarLibros().length === 0) {
        biblioteca.crearLibro("Cien años de soledad", "Gabriel García Márquez", 1967);
        biblioteca.crearLibro("Don Quijote de la Mancha", "Miguel de Cervantes", 1605);
        biblioteca.crearLibro("El Quijote", "Miguel de Cervantes", 1605);
    }
    console.log("\n");
    ...
    console.table(biblioteca.listarLibros().map((l) => ({
        "ID": l.id,
        "Título": l.titulo,
        "Autor": l.autor,
        "Año": l.año,
    ...
    console.table(biblioteca.listarUsuarios().map((u) => ({
        "ID": u.id,
        "Nombre": u.nombre
    })));
    console.log("\n");
    ...
    if (libros.length > 0 && usuarios.length > 0) {
        const resultadoPrestamo = biblioteca.prestarLibro(libros[0].id, usuarios[0].id);
    ...
    console.table(biblioteca.listarPrestamos(true).map((p) => ({
        "ID": p.id,
        "Libro ID": p.libroId,
